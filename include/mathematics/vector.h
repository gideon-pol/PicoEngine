...
struct vec {
    ...
    FORCE_INLINE UNROLL constexpr vec() {
        for (int i = 0; i < C; i++) {
            data[i] = SCAST<T>(0);
        }
    }

    FORCE_INLINE UNROLL constexpr vec(T val){
        for (int i = 0; i < C; i++) {
            data[i] = val;
        }
    };

    FORCE_INLINE UNROLL constexpr vec(const vec<T, C>& other) {
        for (int i = 0; i < C; i++) {
            data[i] = other(i);
        }
    };

    FORCE_INLINE UNROLL constexpr vec(const T (&arr)[C]) {
        for (int i = 0; i < C; i++) {
            data[i] = arr[i];
        }
    };
    ...
    FORCE_INLINE UNROLL constexpr vec(const vec<T2, C>& other) {
        for (int i = 0; i < C; i++) {
    ...
    FORCE_INLINE UNROLL constexpr vec<T, C>& operator=(const vec<T, C>& other) {
        for (int i = 0; i < C; i++) {
            data[i] = other(i);
        }
        return *this;
    ...
    FORCE_INLINE UNROLL constexpr T operator*(const vec<T, C>& other) const {
        T result = SCAST<T>(0);
        for (int i = 0; i < C; i++) {
    ...
    FORCE_INLINE UNROLL constexpr vec<T, C> operator*(const T& other) const {
    ...
    FORCE_INLINE UNROLL constexpr vec<T, C> operator/(const vec<T, C>& other) const {
        vec<T, C> result = vec<T, C>();
        for (int i = 0; i < C; i++) {
    ...
    FORCE_INLINE constexpr vec<T, C> operator-() const {
        vec<T, C> result = vec<T, C>();
        for (int i = 0; i < C; i++) {
            result[i] = -data[i];
        }
    ...
    FORCE_INLINE UNROLL constexpr vec<T, C>& operator*=(const T& other) {
        for (int i = 0; i < C; i++) {
            data[i] *= other;
        }
        return *this;
    ...
    FORCE_INLINE UNROLL constexpr bool operator==(const vec<T, C>& other) const {
        for (int i = 0; i < C; i++) {
            if (data[i] != other(i)) {
                return false;
            }
    ...
    FORCE_INLINE UNROLL constexpr T magnitude() const {
    ...
    FORCE_INLINE UNROLL constexpr vec<T, C> normalize() const {
        vec<T, C> result = vec<T, C>();
        T mag = magnitude();
    ...
    FORCE_INLINE UNROLL constexpr T dot(const vec<T, C>& other) const {
        T result = 0;
        for (int i = 0; i < C; i++) {
            result += data[i] * other(i);
        }
...
struct vec2 : public vec<T, 2> {
    using vec<T, 2>::data;
    using vec<T, 2>::vec;
    FORCE_INLINE constexpr vec2(T x, T y) : vec<T, 2>({x, y}) {};
    FORCE_INLINE constexpr vec2(const vec<T, 2>& other) : vec<T, 2>(other) {};

    template<typename U>
    FORCE_INLINE constexpr vec2(const vec<U, 2>& other) : vec<T, 2>({
...
struct vec3 : public vec<T, 3> {
    using vec<T, 3>::data;
    using vec<T, 3>::vec;
    FORCE_INLINE constexpr vec3(T x, T y, T z) : vec<T, 3>({x, y, z}) {};
    FORCE_INLINE constexpr vec3(const vec<T, 3>& other) : vec<T, 3>(other) {};
    FORCE_INLINE constexpr vec3(const vec2<T>& other, T z) : vec<T, 3>({other(0), other(1), z}) {};
    
    template <typename U>
    FORCE_INLINE constexpr vec3(const vec<U, 3>& other) : vec<T, 3>({
        SCAST<T>(other(0)), SCAST<T>(other(1)), SCAST<T>(other(2))}) {};

    FORCE_INLINE constexpr vec3 cross(const vec3<T>& other) const {
...
struct vec4 : public vec<T, 4> {
    using vec<T, 4>::data;
    using vec<T, 4>::vec;
    FORCE_INLINE constexpr vec4(T x, T y, T z, T w) : vec<T, 4>({x, y, z, w}) {};
    FORCE_INLINE constexpr vec4(const vec<T, 4>& other) : vec<T, 4>(other) { };
    FORCE_INLINE constexpr vec4(const vec3<T>& other, T w) : vec<T, 4>({other(0), other(1), other(2), w}) {};

    template <typename U>
    FORCE_INLINE constexpr vec4(const vec<U, 4>& other) : vec<T, 4>({
    ...
    FORCE_INLINE constexpr vec3<T> homogenize() const {
...
struct vec3f : public vec3<fixed> {
    ...
    FORCE_INLINE constexpr vec3f(fixed x, fixed y, fixed z) : vec3<fixed>(x, y, z) {};
    FORCE_INLINE constexpr vec3f(const vec<fixed, 3>& other) : vec3<fixed>(other) {};
    FORCE_INLINE constexpr vec3f& operator=(const vec3f& other) {
...
struct vec4f : public vec4<fixed> {
    ...
    FORCE_INLINE constexpr vec4f(fixed x, fixed y, fixed z, fixed w) : vec4<fixed>(x, y, z, w) {};
    FORCE_INLINE constexpr vec4f(const vec4<fixed>& other) : vec4<fixed>(other) {};
    FORCE_INLINE constexpr vec4f& operator=(const vec4f& other) {
